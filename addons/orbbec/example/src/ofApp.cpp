	...
	if (astra.isFrameNew() && bDrawPointCloud) {
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
                ...
                // Lets turn on gravity
//                p.y -= 100.1;
//                p.x += ofNoise(ofGetElapsedTimef());
//                p.y += ofNoise(ofGetElapsedTimef());
...
void ofApp::updateHands(){
    ...
    for (auto& hand : astra.getHandsWorld()) {
        ...
        cout << "id = " << hand.first << endl;
        
        string address;
        if(count == 0) address = "first_hand";
        else address = "second_hand";
        
        ofxOscMessage m;
        m.setAddress(address);
        m.addFloatArg(ofMap(pos.x,-500.,500.,0.0,1.0));
        m.addFloatArg(ofMap(pos.y,-250.,350.,0.0,1.0));
        m.addFloatArg(ofMap(pos.z,0.0,4000.,0.0,1.0));
        osc.sendMessage(m);
